...
describe('FieldNameRules', () => {
  ...
  it('should fail on existing with different initial key', () => {
    const existing = ['t1','t2'];
    expect(GetFieldNameRules(existing, 't2', (k: string) => k)[0]('t1')).toEqual(i18nKeys.Rules.FieldAlreadyExists)
  })
  ...
  it('should not fail on existing with same initial key', () => {
    const existing = ['t1','t2'];
    expect(GetFieldNameRules(existing, 't1', (k: string) => k)[0]('t1')).toEqual(true)
  })
  it('should not fail on new', () => {
    const existing = ['t1','t2'];
    expect(GetFieldNameRules(existing, 't1', (k: string) => k)[0]('t3')).toEqual(true)
  })
})
