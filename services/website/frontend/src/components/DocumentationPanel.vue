        ...
        <v-expansion-panel-text>
            ...
            <span v-if="e.Style != 'LINK-HREF' && e.Style != 'LINK-TITLE' && e.Style !== 'FOOTNOTE'"
                  :class="getClass(e.Style)">{{ e.Content }}</span>
                        <a v-if="e.Style == 'LINK-HREF'" :href="e.Content" target="_blank">{{
                                g.Elements[j - 1].Content
                            }}</a>
              ...
              {{ e.Content }}
                ...
                <h4 v-if="g.Type === 'SUBSECTION'">{{ g.Elements[0].Content }}</h4>
            </template>
            ...
            <span v-if="e.Style != 'LINK-HREF' && e.Style != 'LINK-TITLE' && e.Style !== 'FOOTNOTE'"
                  :class="getClass(e.Style)">{{ e.Content }}</span>
                            <a v-if="e.Style == 'LINK-HREF'" :href="e.Content"
                               target="_blank">{{ f.Elements[j - 1].Content }}</a>
                        </template>
                    </li>
                </ul>
            </div>
