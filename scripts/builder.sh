...
_build_mac_target() {
    _build_target_for_platform $1 @io_bazel_rules_go//go/toolchain:darwin_amd64
}

# Build the given target ($1) for mac x64 m1
_build_mac_m1_target() {
    _build_target_for_platform $1 @io_bazel_rules_go//go/toolchain:darwin_arm64
}
...
# This also renames the file to the given final name ($2)
_copy_mac_exe() {
    _copy_target_executable $1 $2 @io_bazel_rules_go//go/toolchain:darwin_amd64
}

# Copy the given macOS m1 target ($1) to a well known location ($2).
# This also renames the file to the given final name ($2)
_copy_mac_m1_exe() {
    _copy_target_executable $1 $2 @io_bazel_rules_go//go/toolchain:darwin_arm64
}
...
build_mac_m1_target() {
  _build_mac_m1_target "$1"
  _copy_mac_m1_exe "$1" "$2"
}
